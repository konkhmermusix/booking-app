    ...
    <div class="container mx-auto px-4">
                    ...
                    <div class="flex items-baseline justify-between border-b border-gray-50 dark:border-gray-800/60 pb-4">
                        <div class="flex items-baseline gap-1">
                            <h2 class="text-2xl md:text-3xl font-black text-blue-600 dark:text-blue-400">
                                 ${{ $promotion->discounted_price == floor($promotion->discounted_price) ? number_format($promotion->discounted_price, 0) : number_format($promotion->discounted_price, 2) }}
                            </h2>
                            <p class="text-gray-500 dark:text-gray-400 text-sm">/ {{ $roomType->category === 'stay' ? 'យប់' : 'ម៉ោង' }}</p>
                        </div>
                        <div class="text-right">
                            <span class="text-gray-400 line-through text-base font-bold">${{ $promotion->original_price == floor($promotion->original_price) ? number_format($promotion->original_price, 0) : number_format($promotion->original_price, 2) }}</span>
                            <p class="text-red-500 text-xs font-bold bg-red-50 dark:bg-red-950/40 px-2 py-0.5 rounded-md mt-0.5">
                                @php $saved = $promotion->original_price - $promotion->discounted_price; @endphp
                                សន្សំបាន ${{ $saved == floor($saved) ? number_format($saved, 0) : number_format($saved, 2) }}
                            </p>
                        </div>
                    </div>
                    ...
                    <form action="{{ $roomType->category === 'stay' ? route('promotion.addhotelpro') : route('promotion.addmeetingpro') }}" method="POST" class="space-y-4">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <input type="date" name="check_in" id="check_in" min="{{ date('Y-m-d') }}" value="{{ $check_in ?? session('search_check_in', date('Y-m-d')) }}" required
                            ...
                            <div class="space-y-1.5">
                                ...
                                <input type="date" name="check_out" id="check_out" min="{{ date('Y-m-d', strtotime('+1 day')) }}" value="{{ $check_out ?? session('search_check_out', date('Y-m-d', strtotime('+1 day'))) }}" required
                                    class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-3.5 rounded-xl focus:ring-2 ring-blue-500 outline-none text-gray-900 dark:text-white text-sm h-[52px]">
                            </div>
                            @else
                            <div class="space-y-1.5">
                                ...
                                <input type="date" name="start_date" id="start_date" min="{{ date('Y-m-d') }}" value="{{ $check_in ?? session('search_check_in', date('Y-m-d')) }}" required
                                    class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-3.5 rounded-xl focus:ring-2 ring-blue-500 outline-none text-gray-900 dark:text-white text-sm h-[52px]">
                            </div>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <input type="date" name="end_date" id="end_date" value="{{ $check_out ?? session('search_check_out', date('Y-m-d')) }}" required
                                    class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-3.5 rounded-xl focus:ring-2 ring-blue-500 outline-none text-gray-900 dark:text-white text-sm h-[52px]">
                            </div>
        ...
        <div class="mt-8 bg-white dark:bg-gray-900 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800 p-5 md:p-8 transition-colors duration-300">
            <div class="flex items-center gap-2 mb-2">
                <span class="bg-red-500 text-white px-3 py-1 rounded-xl text-xs font-bold uppercase">
                    PROMOTION
                </span>
                <span class="text-xs text-red-500 font-semibold">
                    <i class="fas fa-clock mr-1"></i> ផុតកំណត់៖ {{ \Carbon\Carbon::parse($promotion->expiry_date)->locale('km')->diffForHumans() }}
                </span>
            </div>

            <h1 class="text-2xl md:text-4xl font-black mb-4 text-gray-900 dark:text-white">
                {{ $promotion->title }} ({{ $roomType->name }})
            </h1>
                ...
                <div class="lg:col-span-2 space-y-5" id="reviewsContainer">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800/40 p-4 rounded-2xl border border-gray-100 dark:border-gray-800/60">
                        ...
                        <div id="edit-box-{{ $review->id }}" class="hidden mt-2 pl-12">
                            <form id="edit-form-{{ $review->id }}" action="{{ route('frontend.room_details.update', $review->id) }}" method="POST" class="space-y-2 edit-review-form">
                                @csrf
                                @method('PUT')
                                <div class="flex items-center gap-2 mb-2">
                        ...
                        <div class="mt-3 pl-12 flex items-center gap-4">
                            ...
                            <form action="{{ route('frontend.room_details.delete', $review->id) }}" method="POST" class="inline delete-review-form">
                                @csrf
                                @method('DELETE')
                        ...
                        <div id="reply-box-{{ $review->id }}" class="hidden mt-4 pl-12">
                            <form action="{{ route('frontend.room_details.replay', $roomType->id) }}" method="POST" class="flex gap-2 reply-review-form">
                                @csrf
                                <input type="hidden" name="room_type_id" value="{{ $roomType->id }}">
                        ...
                        <div class="mt-4 pl-12 space-y-3 border-l-2 border-gray-200 dark:border-gray-700">
                            ...
                            <div class="bg-white dark:bg-gray-900/60 p-3 rounded-xl border border-gray-100/80 dark:border-gray-800/40 ml-2">
                                ...
                                <div id="edit-reply-box-{{ $reply->id }}" class="hidden mt-2 pl-9">
                                    <form id="edit-reply-form-{{ $reply->id }}" action="{{ route('frontend.room_details.update', $reply->id) }}" method="POST" class="space-y-2 edit-reply-form">
                                        @csrf
                                        @method('PUT')
                                ...
                                <div class="mt-2 pl-9 flex items-center gap-3 border-t border-gray-50 dark:border-gray-800 pt-1.5">
                                    ...
                                    <form action="{{ route('frontend.room_details.delete', $reply->id) }}" method="POST" class="inline delete-reply-form">
                                        @csrf
                                        @method('DELETE')
...
<script>
    ...
    function calculatePromoPrice() {
        ...
        const price = parseFloat("{{ $promotion->discounted_price }}") || 0;
        ...
        if (category === 'stay') {
            ...
            display.innerHTML = `<i class="fas fa-moon mr-2 text-blue-500"></i> ${nights} យប់ &times; $${price.toLocaleString()} = <span class="ml-auto font-black text-lg text-blue-600 dark:text-blue-400">$${total.toLocaleString()}</span>`;
        } else {
            ...
            wrapper.classList.remove('hidden');
            display.innerHTML = `<i class="fas fa-clock mr-2 text-blue-500"></i> ${totalHours} ម៉ោង &times; $${price.toLocaleString()} = <span class="ml-auto font-black text-lg text-blue-600 dark:text-blue-400">$${total.toLocaleString()}</span>`;
        }
    }
    ...
    if (reviewForm) {
        reviewForm.addEventListener('submit', function(e) {
            ...
            fetch("{{ route('frontend.room_details.store', $roomType->id) }}", {
                method: 'POST',
                body: formData,
                headers: {

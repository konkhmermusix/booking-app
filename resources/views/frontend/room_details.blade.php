@extends('layouts.app')

@section('title', $roomType->name . ' - ព័ត៌មានលម្អិត')

@section('content')

<style>
    .scrollbar-hide::-webkit-scrollbar {
        display: none;
    }

    .scrollbar-hide {
        scrollbar-width: none;
        -ms-overflow-style: none;
    }
</style>

<div class="min-h-screen bg-gray-50 pb-20">
    <div class="container mx-auto px-3 sm:px-4 lg:px-6 pt-6">

        {{-- TOP SECTION --}}
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">

            {{-- LEFT IMAGE SECTION --}}
            <div class="lg:col-span-2">

                <div class="space-y-4">

                    {{-- MAIN IMAGE --}}
                    <div class="relative overflow-hidden rounded-2xl md:rounded-3xl shadow-xl group">

                        <img id="mainImage"
                            src="{{ asset('storage/' . $roomType->images->first()->image_path) }}"
                            class="w-full h-64 sm:h-80 md:h-[420px] lg:h-[520px] object-cover duration-700 group-hover:scale-105">

                        {{-- ZOOM BUTTON --}}
                        <button type="button"
                            onclick="openImageModal()"
                            class="absolute top-3 right-3 md:top-4 md:right-4 bg-white/90 backdrop-blur px-3 py-2 rounded-xl shadow hover:bg-blue-600 hover:text-white duration-300">
                            <i class="fas fa-expand"></i>
                        </button>

                    </div>

                    {{-- THUMBNAILS --}}
                    <div class="relative">

                        {{-- LEFT --}}
                        <button type="button"
                            onclick="scrollThumbs(-1)"
                            class="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg w-9 h-9 md:w-11 md:h-11 rounded-full flex items-center justify-center hover:bg-blue-600 hover:text-white">
                            <i class="fas fa-chevron-left text-sm"></i>
                        </button>

                        {{-- THUMB CONTAINER --}}
                        <div id="thumbContainer"
                            class="flex gap-2 md:gap-3 overflow-x-auto scroll-smooth px-10 md:px-12 py-2 scrollbar-hide">

                            @foreach($roomType->images as $index => $img)

                            <img
                                src="{{ asset('storage/' . $img->image_path) }}"
                                onclick="changeImage(this)"
                                class="thumbItem flex-shrink-0
                                w-24 h-16
                                sm:w-28 sm:h-20
                                md:w-36 md:h-24
                                rounded-xl md:rounded-2xl
                                object-cover cursor-pointer
                                border-2
                                {{ $index == 0 ? 'border-blue-600' : 'border-transparent' }}
                                hover:border-blue-600
                                hover:scale-105 duration-300 shadow-md">

                            @endforeach

                        </div>

                        {{-- RIGHT --}}
                        <button type="button"
                            onclick="scrollThumbs(1)"
                            class="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-white shadow-lg w-9 h-9 md:w-11 md:h-11 rounded-full flex items-center justify-center hover:bg-blue-600 hover:text-white">
                            <i class="fas fa-chevron-right text-sm"></i>
                        </button>

                    </div>

                </div>

            </div>

            {{-- RIGHT BOOKING --}}
            <div>

                <div class="sticky top-20 bg-white rounded-2xl md:rounded-3xl shadow-xl p-4 md:p-6 space-y-5">

                    <div>
                        <h2 class="text-2xl md:text-3xl font-black text-blue-600">
                            ${{ number_format($roomType->base_price,0) }}
                        </h2>
                        <p class="text-gray-500 text-sm">/ យប់</p>
                    </div>

                    <div id="totalPriceWrapper" class="hidden">
                        <span id="totalPriceDisplay"
                            class="inline-block bg-green-100 text-green-700 px-4 py-2 rounded-xl font-bold text-sm">
                        </span>
                    </div>

                    <form action="{{ route('booking.store') }}" method="POST">
                        @csrf

                        <!-- បន្ថែម Hidden Input ឱ្យគ្រប់គ្រាន់ -->
                        <input type="hidden" name="hotel_id" value="{{ $roomType->hotel_id }}">
                        <input type="hidden" name="room_type_id" value="{{ $roomType->id }}">
                        <input type="hidden" name="price_at_booking" value="{{ $roomType->base_price }}">
                        <input type="hidden" name="room_id" value="{{ $roomType->id }}">

                        <div class="space-y-4">
                            <div>
                                <label class="text-sm font-semibold">ថ្ងៃចូល</label>
                                <!-- បន្ថែម name="check_in" -->
                                <input type="date" name="check_in" id="check_in" min="{{ date('Y-m-d') }}" value="{{ date('Y-m-d') }}"
                                    class="w-full bg-gray-50 dark:bg-gray-800 border-none p-3.5 rounded-xl focus:ring-2 ring-blue-500 outline-none dark:text-white text-sm h-[52px]"
                                    required>
                            </div>

                            <div>
                                <label class="text-sm font-semibold">ថ្ងៃចេញ</label>
                                <!-- បន្ថែម name="check_out" -->
                                <input type="date" name="check_out" id="check_out" min="{{ date('Y-m-d', strtotime('+1 day')) }}" value="{{ date('Y-m-d', strtotime('+1 day')) }}"
                                    class="w-full bg-gray-50 dark:bg-gray-800 border-none p-3.5 rounded-xl focus:ring-2 ring-blue-500 outline-none dark:text-white text-sm h-[52px]"
                                    required>
                            </div>

                            <div>
                                <label class="text-sm font-semibold">ភ្ញៀវ</label>
                                <select name="guests" class="w-full bg-gray-50 dark:bg-gray-800 border-none px-4 rounded-xl focus:ring-2 ring-blue-500 outline-none dark:text-white appearance-none cursor-pointer transition-all font-medium text-sm h-[52px]">
                                    @for($i=1; $i <= $roomType->max_guests; $i++)
                                        <option value="{{ $i }}">{{ $i }} នាក់</option>
                                        @endfor
                                </select>
                            </div>

                            <div>
                                <label class="text-sm font-semibold">សំណូមពរពិសេស (Special Requests)</label>
                                <textarea name="special_requests" rows="2"
                                    class="w-full bg-gray-50 dark:bg-gray-800 border-none p-3.5 rounded-xl focus:ring-2 ring-blue-500 outline-none dark:text-white text-sm"
                                    placeholder="ឧទាហរណ៍៖ សុំបន្ទប់ស្ងាត់ ឬបន្ថែមគ្រែ..."></textarea>
                            </div>

                            <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 md:py-4 rounded-2xl font-black transition-all active:scale-95">
                                កក់បន្ទប់ស្នាក់នៅ
                            </button>
                        </div>
                    </form>
                </div>
            </div>
        </div>

        {{-- DETAILS --}}
        <div class="mt-10 md:mt-14 bg-white rounded-2xl md:rounded-3xl shadow-lg p-5 md:p-8">

            <h1 class="text-2xl md:text-4xl font-black mb-4">
                {{ $roomType->name }}
            </h1>

            <div class="flex flex-wrap gap-3 mb-8">
                <span class="px-4 py-2 bg-gray-100 rounded-xl text-sm">
                    {{ $roomType->beds }} គ្រែ
                </span>

                <span class="px-4 py-2 bg-gray-100 rounded-xl text-sm">
                    {{ $roomType->max_guests }} នាក់
                </span>

                <span class="px-4 py-2 bg-yellow-100 rounded-xl text-sm">
                    ⭐ {{ number_format($roomType->reviews->avg('rating') ?? 0,1) }}
                </span>
            </div>

            <h2 class="text-xl md:text-2xl font-bold mb-3">ពណ៌នាអំពីបន្ទប់</h2>

            <p class="text-gray-600 leading-relaxed mb-10">
                {{ $roomType->description }}
            </p>

            {{-- FACILITIES --}}
            <h2 class="text-xl md:text-2xl font-bold mb-5">គ្រឿងបរិក្ខារ</h2>

            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">

                @foreach($roomType->facilities as $facility)

                <div class="bg-gray-50 rounded-2xl p-4 flex items-center gap-3 text-sm md:text-base">
                    <i class="{{ $facility->icon ?? 'fas fa-check' }} text-blue-600"></i>
                    <span>{{ $facility->name }}</span>
                </div>

                @endforeach

            </div>

        </div>

    </div>
</div>

{{-- IMAGE MODAL --}}
<div id="imageModal"
    class="fixed inset-0 z-[9999] hidden bg-black/95 items-center justify-center p-4">

    <button onclick="closeImageModal()"
        class="absolute top-5 right-6 text-white text-5xl hover:text-red-400">
        &times;
    </button>

    <img id="modalImage"
        class="max-w-full max-h-[92vh] rounded-2xl object-contain shadow-2xl">

</div>

<script>
    document.addEventListener('DOMContentLoaded', function() {

        const checkIn = document.getElementById('check_in');
        const checkOut = document.getElementById('check_out');

        checkIn.addEventListener('change', calculatePrice);
        checkOut.addEventListener('change', calculatePrice);

        calculatePrice();
    });

    function changeImage(el) {
        document.getElementById('mainImage').src = el.src;
        document.getElementById('modalImage').src = el.src;

        document.querySelectorAll('.thumbItem').forEach(item => {
            item.classList.remove('border-blue-600');
            item.classList.add('border-transparent');
        });

        el.classList.remove('border-transparent');
        el.classList.add('border-blue-600');
    }

    function scrollThumbs(direction) {
        document.getElementById('thumbContainer').scrollBy({
            left: direction * 260,
            behavior: 'smooth'
        });
    }

    function openImageModal() {
        const modal = document.getElementById('imageModal');
        const img = document.getElementById('mainImage').src;

        document.getElementById('modalImage').src = img;

        modal.classList.remove('hidden');
        modal.classList.add('flex');

        document.body.classList.add('overflow-hidden');
    }

    function closeImageModal() {
        const modal = document.getElementById('imageModal');

        modal.classList.add('hidden');
        modal.classList.remove('flex');

        document.body.classList.remove('overflow-hidden');
    }

    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') {
            closeImageModal();
        }
    });

    function calculatePrice() {
        const checkIn = document.getElementById('check_in').value;
        const checkOut = document.getElementById('check_out').value;

        const wrapper = document.getElementById('totalPriceWrapper');
        const display = document.getElementById('totalPriceDisplay');

        let basePrice = {
            {
                $roomType - > base_price
            }
        };

        if (checkIn && checkOut) {
            let start = new Date(checkIn);
            let end = new Date(checkOut);

            let diff = (end - start) / (1000 * 60 * 60 * 24);

            if (diff > 0) {
                let total = diff * basePrice;

                wrapper.classList.remove('hidden');

                display.innerHTML =
                    `តម្លៃសរុប: $${total.toLocaleString()} (${diff} យប់)`;
            } else {
                wrapper.classList.add('hidden');
            }
        }
    }
</script>


<script>
    function changeImage(el) {
        document.getElementById('mainImage').src = el.src;
        document.getElementById('modalImage').src = el.src;

        document.querySelectorAll('.thumbItem').forEach(item => {
            item.classList.remove('border-blue-600');
        });

        el.classList.add('border-blue-600');
    }

    function scrollThumbs(direction) {
        document.getElementById('thumbContainer').scrollBy({
            left: direction * 280,
            behavior: 'smooth'
        });
    }

    function openImageModal() {
        let img = document.getElementById('mainImage').src;

        document.getElementById('modalImage').src = img;
        document.getElementById('imageModal').classList.remove('hidden');
        document.getElementById('imageModal').classList.add('flex');
    }

    function closeImageModal() {
        document.getElementById('imageModal').classList.add('hidden');
        document.getElementById('imageModal').classList.remove('flex');
    }

    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape') {
            closeImageModal();
        }
    });
</script>


@endsection
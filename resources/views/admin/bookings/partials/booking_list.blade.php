<div x-show="viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" x-transition>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all group border border-transparent dark:border-gray-800 relative overflow-hidden">
        ...
        <div class="h-40 -mx-5 -mt-5 mb-4 overflow-hidden relative group/img">
            @if($booking->room && $booking->room->roomType && $booking->room->roomType->images->count() > 0)
            ...
            <div class="absolute inset-0 bg-black/40 flex items-center justify-center gap-2 opacity-0 group-hover/img:opacity-100 transition-opacity">
                <button @click="currentBooking = {{ $booking->toJson() }}; showDetailModal = true" class="w-9 h-9 bg-white text-blue-600 rounded-xl hover:scale-110 transition flex items-center justify-center">
                    <i class="fas fa-eye"></i>
                </button>
                <button @click="currentBooking = {{ $booking->toJson() }}; showEditModal = true" class="w-9 h-9 bg-white text-amber-500 rounded-xl hover:scale-110 transition flex items-center justify-center">
        ...
        <div class="space-y-3">
            ...
            <div class="flex justify-between items-center">
                <span class="text-xs font-bold text-gray-500 dark:text-gray-400 italic">
                    ភ្ញៀវ: {{ $booking->user ? $booking->user->name . ' (អនឡាញ)' : 'ភ្ញៀវមកផ្ទាល់' }}
                </span>
...
<div x-show="viewMode === 'list'" class="space-y-3" x-transition>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all border border-transparent dark:border-gray-800">
        <div class="flex items-center gap-4">
            <div class="w-14 h-14 rounded-xl bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-blue-600 dark:text-blue-400 font-black text-xs">
                #{{ substr($booking->booking_code, -4) }}
            </div>
            <div>
                <h4 class="font-bold text-sm text-gray-800 dark:text-gray-100 uppercase">
                    បន្ទប់ {{ $booking->room->room_number ?? 'N/A' }} - {{ $booking->user ? $booking->user->name . ' (អនឡាញ)' : 'ភ្ញៀវមកផ្ទាល់' }}
                </h4>
                <p class="text-[10px] text-gray-400 italic">
                    {{ $booking->check_in }} ដល់ {{ $booking->check_out }} | សរុប: ${{ number_format($booking->total_price, 2) }}
                </p>
            </div>
        </div>
        <div class="flex items-center gap-4">
            ...
            <div class="flex gap-1">
                <button @click="currentBooking = {{ $booking->toJson() }}; showDetailModal = true" class="p-2 text-gray-400 hover:text-blue-500 transition-colors">
                    <i class="fas fa-eye text-sm"></i>
                </button>
                <button @click="currentBooking = {{ $booking->toJson() }}; showEditModal = true" class="p-2 text-gray-400 hover:text-amber-500 transition-colors">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800 text-sm">
                ...
                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        ...
                        <div class="text-[11px] text-gray-400">បន្ទប់ {{ $booking->room->room_number ?? 'N/A' }}</div>
                    </td>
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2">
                            <button @click="currentBooking = {{ $booking->toJson() }}; showDetailModal = true"
                                class="p-2 text-gray-400 hover:text-blue-500 transition-colors">
                                <i class="fas fa-eye text-sm"></i>
                            </button>
                            <button @click="currentBooking = {{ $booking->toJson() }}; showEditModal = true"
...
<div class="mt-4 bg-white dark:bg-gray-800 p-3 rounded-2xl shadow-sm border border-transparent dark:border-gray-800">

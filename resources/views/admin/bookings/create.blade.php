...
@section('title', 'បង្កើតការកក់ថ្មី')
...
<div class="p-2 sm:p-2 max-w-4xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-xl font-bold dark:text-white">បង្កើតការកក់បន្ទប់ថ្មី</h2>
            <p class="text-xs text-gray-400">Add a new room reservation to the system</p>
        </div>
        ...
        <form action="{{ route('bookings.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="room_id" id="room_select" class="w-full h-11 px-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm dark:text-white focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all @error('room_id') border-rose-500 focus:ring-rose-500/50 @enderror">
                        ...
                        <option value="{{ $room->id }}" data-price="{{ $room->roomType->price ?? 0 }}" {{ old('room_id') == $room->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="number" name="number_of_guests" value="{{ old('number_of_guests', 1) }}" min="1"
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="total_price" id="total_price" value="{{ old('total_price') }}"
                ...
                <div>
                    ...
                    <select name="payment_status" class="w-full h-11 px-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm dark:text-white focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all @error('payment_status') border-rose-500 focus:ring-rose-500/50 @enderror">
                        <option value="pending" {{ old('payment_status') === 'pending' ? 'selected' : '' }}>រង់ចាំ (Pending)</option>
                        <option value="paid" {{ old('payment_status') === 'paid' ? 'selected' : '' }}>បានបង់រួច (Paid)</option>
                        <option value="failed" {{ old('payment_status') === 'failed' ? 'selected' : '' }}>បរាជ័យ (Failed)</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="booking_status" class="w-full h-11 px-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm dark:text-white focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all @error('booking_status') border-rose-500 focus:ring-rose-500/50 @enderror">
                        <option value="pending" {{ old('booking_status') === 'pending' ? 'selected' : '' }}>រង់ចាំពិនិត្យ (Pending)</option>
                        <option value="confirmed" {{ old('booking_status') === 'confirmed' ? 'selected' : '' }}>បានបញ្ជាក់ (Confirmed)</option>
                        <option value="completed" {{ old('booking_status') === 'completed' ? 'selected' : '' }}>រួចរាល់ (Completed)</option>
                        <option value="cancelled" {{ old('booking_status') === 'cancelled' ? 'selected' : '' }}>បានបោះបង់ (Cancelled)</option>
                    </select>
            ...
            <div>
                ...
                <textarea name="notes" rows="4" placeholder="មតិផ្សេងៗរបស់អតិថិជន..."
                    class="w-full p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-sm dark:text-white focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all @error('notes') border-rose-500 focus:ring-rose-500/50 @enderror">{{ old('notes') }}</textarea>
                @error('notes')
                <p class="text-rose-500 text-xs mt-1 font-semibold">{{ $message }}</p>
                @enderror
            </div>

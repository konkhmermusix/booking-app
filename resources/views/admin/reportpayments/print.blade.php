...
<head>
    ...
    <title>របាយការណ៍ប្រតិបត្តិការបង់ប្រាក់ - PNT Palace Hotel</title>
...
<body class="p-8 max-w-4xl mx-auto">
    ...
    <div class="no-print mb-6 flex justify-between items-center bg-gray-100 p-4 rounded-2xl border border-gray-200">
        <div>
            <h1 class="font-bold text-gray-800 text-sm">ផ្ទាំងព្រីនរបាយការណ៍បង់ប្រាក់ (Print & Export Preview)</h1>
    ...
    <div class="flex justify-between items-center border-b-2 border-gray-800 pb-4 mb-6">
        ...
        <div class="text-right">
            <div class="inline-block p-3 bg-blue-50 border border-blue-200 rounded-xl mb-1">
                <h3 class="text-base font-black text-blue-900">របាយការណ៍ប្រតិបត្តិការបង់ប្រាក់</h3>
            </div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        ...
        <div class="border border-gray-300 p-3 rounded-xl text-center bg-gray-50">
            <span class="text-[11px] text-gray-500 uppercase font-bold">ប្រាក់ប្រមូលបានជោគជ័យ</span>
            <p class="text-xl font-black text-emerald-600 mt-0.5">${{ number_format($totalPaidAmount, 2) }}</p>
        </div>
        <div class="border border-gray-300 p-3 rounded-xl text-center bg-gray-50">
            <span class="text-[11px] text-gray-500 uppercase font-bold">ប្រាក់រង់ចាំ (Pending)</span>
            <p class="text-xl font-black text-amber-600 mt-0.5">${{ number_format($payments->where('status', 'pending')->sum('amount'), 2) }}</p>
        </div>
    </div>
        ...
        <tbody class="divide-y divide-gray-300">
            @forelse($payments as $index => $p)
                ...
                <tr>
                    ...
                    <td class="p-2.5 border-r border-gray-300 font-bold text-blue-900">#{{ $p->booking_code }}</td>
                    ...
                    <td class="p-2.5 text-right font-bold text-blue-900">${{ number_format($p->amount, 2) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="8" class="p-4 text-center text-gray-400 italic">មិនទាន់មានទិន្នន័យបង់ប្រាក់ឡើយ</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot>
            <tr class="bg-gray-100 font-bold text-gray-800 border-t border-gray-400">
                <td colspan="7" class="p-3 text-right uppercase">សរុបទឹកប្រាក់បង់រួចសរុប (Total Paid):</td>
                <td class="p-3 text-right text-blue-950 font-black text-sm">${{ number_format($totalPaidAmount, 2) }}</td>
            </tr>
        </tfoot>
    </table>
    ...
    <div class="grid grid-cols-2 gap-8 pt-8 text-center text-xs">
        ...
        <div>
            <p class="font-bold text-gray-700 mb-16">ប្រធានផ្នែកបេឡា / អ្នកអនុម័ត (Approved By)</p>
            <p class="font-semibold text-gray-900">......................................................</p>
            <p class="text-[11px] text-gray-500 mt-1">ថ្ងៃទី........ ខែ........ ឆ្នាំ២០....</p>
        </div>
    </div>

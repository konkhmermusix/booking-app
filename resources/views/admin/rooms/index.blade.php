...
<div class="space-y-6" x-data="{ viewMode: localStorage.getItem('roomView') || 'list', showAddModal: false, showEditModal: false,  showDetailModal: false, currentRoom: { hotel: {}, room_type: {}}}">
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white dark:bg-gray-900 p-5 rounded-[2rem] border dark:border-gray-800 shadow-sm">
        <div class="shrink-0">
            <h2 class="text-2xl font-bold dark:text-white tracking-tight">គ្រប់គ្រងបន្ទប់</h2>
        </div>

        <div class="flex flex-wrap items-center gap-3 w-full lg:w-auto">
            <form action="{{ route('rooms.index') }}" method="GET" class="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                ...
                <div class="w-full sm:w-48">
                    <select name="status" onchange="this.form.submit()"
                        ...
                        <option value="available" {{ request('status') == 'available' ? 'selected' : '' }}>ស្ថានភាពទំនេរ</option>
                        <option value="booked" {{ request('status') == 'booked' ? 'selected' : '' }}>ស្ថានភាពមានភ្ញៀវ</option>
                        <option value="maintenance" {{ request('status') == 'maintenance' ? 'selected' : '' }}>ស្ថានភាពជួសជុល</option>
                    </select>
                </div>
            </form>

            <div class="flex bg-gray-100 dark:bg-gray-800/50 p-1.5 rounded-2xl h-[52px] items-center">
                <button @click="viewMode = 'list'; localStorage.setItem('roomView', 'list')"
                ...
                <button @click="viewMode = 'grid'; localStorage.setItem('roomView', 'grid')"
            ...
            <button @click="showAddModal = true"
                ...
                <span class="whitespace-nowrap">បន្ថែមបន្ទប់</span>
            </button>
        ...
        <div x-show="viewMode === 'list'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" class="bg-white dark:bg-gray-900 rounded-3xl border dark:border-gray-800 overflow-hidden shadow-sm">
            <table class="w-full text-left">
                <thead class="bg-gray-50 dark:bg-gray-800/50 text-gray-500 dark:text-gray-400 text-xs uppercase font-bold tracking-wider">
                    <tr>
                        <th class="px-6 py-5">លេខបន្ទប់ / ជាន់</th>
                        <th class="px-6 py-5">សណ្ឋាគារ</th>
                        <th class="px-6 py-5">ប្រភេទបន្ទប់</th>
                ...
                <tbody class="divide-y dark:divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="font-bold dark:text-white group-hover:text-blue-600 transition-colors">#{{ $room->room_number }}</div>
                            <div class="text-xs text-gray-400">ជាន់ទី {{ $room->floor ?? 'N/A' }}</div>
                        </td>
                        <td class="px-6 py-4 dark:text-gray-300 font-medium">{{ $room->hotel->name }}</td>
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 rounded-lg text-xs font-bold uppercase">
                                {{ $room->roomType->name }}
                            </span>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <button @click="currentRoom = {{ $room->toJson() }}; showDetailModal = true"
                                ...
                                <form action="{{ route('rooms.destroy', $room->id) }}" method="POST" class="inline delete-form">
                                    @csrf
                                    @method('DELETE')
        ...
        <div x-show="viewMode === 'grid'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-[2.5rem] border dark:border-gray-800 overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all group shadow-sm">
                <div class="relative h-44 bg-gray-100 dark:bg-gray-800 flex items-center justify-center overflow-hidden">
                    ...
                    <img src="{{ asset('storage/' . $room->roomType->images->first()->image_path) }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                    @else
                    <span class="text-4xl font-black text-gray-300 dark:text-gray-700">#{{ $room->room_number }}</span>
                    @endif
                ...
                <div class="p-6">
                    <div class="flex justify-between items-start mb-4">
                        <div>
                            <h3 class="text-xl font-black dark:text-white">បន្ទប់ #{{ $room->room_number }}</h3>
                            <p class="text-xs text-gray-400 font-medium uppercase tracking-widest">{{ $room->hotel->name }}</p>
                        </div>
                        <span class="text-[10px] font-bold px-2 py-1 bg-gray-100 dark:bg-gray-800 dark:text-gray-400 rounded-md">FLOOR {{ $room->floor }}</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between border-t dark:border-gray-800 pt-4 mt-4">
                        ...
                        <div class="flex gap-1">
                            <button @click="currentRoom = {{ json_encode($room->load(['roomType.facilities', 'roomType.images', 'hotel'])) }}; showDetailModal = true"
                                class="p-2 text-gray-400 hover:text-blue-500">
                                <i class="fas fa-eye"></i>
                            </button>

                            <button @click="currentRoom = {{ json_encode($room->load(['roomType.facilities', 'roomType.images', 'hotel'])) }}; showEditModal = true"

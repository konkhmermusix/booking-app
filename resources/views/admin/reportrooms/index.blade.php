...
@section('title', 'របាយការណ៍កក់បន្ទប់')
@section('content')
<style>
    @media print {
        body * {
...
<div id="reportrooms-page" class="p-2 sm:p-2" x-data="{ 
        showDetailModal: false, showCancelModal: false,
        ...
        async fetchReports(url = null) {
            this.loading = true;
            let fetchUrl = url || '{{ route('reportrooms.index') }}';
            try {
        ...
        resetFilters() {
            ...
            this.date_range = '';
            this.per_page = '10';
            this.fetchReports('{{ route('reportrooms.index') }}');
        },
    ...
    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white dark:bg-gray-900 p-4 rounded-2xl shadow-sm mb-6 border border-gray-100 dark:border-gray-800 print-hide">
        <div class="shrink-0">
            <h2 class="text-base font-bold text-gray-800 dark:text-white tracking-tight">របាយការណ៍កក់បន្ទប់សណ្ឋាគារ</h2>
            <p class="text-[10px] text-gray-400 uppercase tracking-widest font-semibold">Live Room Booking Report</p>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2 w-full xl:w-auto">
            ...
            <a :href="'{{ route('reportrooms.export-pdf') }}?search=' + search + '&status=' + status + '&date_range=' + date_range + '&per_page=' + per_page" target="_blank" class="h-9 px-3 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition active:scale-95 shadow-sm shrink-0">
    ...
    <div id="report-container" :class="loading ? 'opacity-40 pointer-events-none' : ''" class="transition-opacity duration-300">
        @include('admin.reportrooms.partials.report_content')
    </div>

    <!-- Detail Modal -->
    <div x-show="showDetailModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div @click.away="showDetailModal = false" class="bg-white dark:bg-gray-900 w-full max-w-lg rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 p-6 space-y-4">
            <div class="flex justify-between items-center border-b border-gray-100 dark:border-gray-800 pb-3">
                <h3 class="text-base font-bold text-gray-800 dark:text-white flex items-center gap-2">
                    <i class="fas fa-file-invoice text-blue-500"></i> លម្អិតវិក្កយបត្រកក់បន្ទប់
                </h3>
                ...
                <div class="space-y-3 text-xs">
                    <div class="grid grid-cols-2 gap-3 bg-gray-50 dark:bg-gray-800/50 p-3 rounded-2xl">
                        <div>
                            ...
                            <p class="font-mono font-bold text-blue-600" x-text="currentBooking.booking_code"></p>
                        </div>
                        <div>
                            <span class="text-gray-400">ស្ថានភាព:</span>
                            <p class="font-bold text-emerald-500 capitalize" x-text="currentBooking.status === 'pending' ? 'រង់ចាំ' : (currentBooking.status === 'confirmed' ? 'បានបញ្ជាក់' : (currentBooking.status === 'completed' ? 'បានបញ្ចប់' : 'បានបោះបង់'))"></p>
                        </div>
                        <div>
                            <span class="text-gray-400">ឈ្មោះអតិថិជន:</span>
                            <p class="font-bold text-gray-800 dark:text-white" x-text="currentBooking.customer_name || (currentBooking.user ? currentBooking.user.name : 'N/A')"></p>
                        </div>
                        <div>
                            <span class="text-gray-400">លេខទូរស័ព្ទ:</span>
                            <p class="font-mono text-gray-700 dark:text-gray-300" x-text="currentBooking.customer_phone || (currentBooking.user ? currentBooking.user.phone : 'N/A')"></p>
                        </div>
                    </div>

                    <div class="space-y-2">
                        <div class="flex justify-between border-b dark:border-gray-800 pb-1">
                            <span class="text-gray-400">ចូល - ចេញ:</span>
                            <span class="font-mono font-bold text-blue-600 dark:text-blue-400" x-text="formatDisplayDate(currentBooking.check_in) + ' ដល់ ' + formatDisplayDate(currentBooking.check_out)"></span>
                        </div>
                        <div class="flex justify-between border-b dark:border-gray-800 pb-1">
                            <span class="text-gray-400">តម្លៃសរុប:</span>
                            <span class="font-black text-emerald-500 text-sm" x-text="'$' + parseFloat(currentBooking.total_price).toFixed(2)"></span>
                        </div>
                    </div>
                </div>
            </template>
    ...
    <!-- Cancel Reason Modal -->
    <div x-show="showCancelModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div @click.away="showCancelModal = false" class="bg-white dark:bg-gray-900 w-full max-w-md rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 p-5 space-y-3">
            <div class="flex justify-between items-center border-b border-gray-100 dark:border-gray-800 pb-2">
                <h3 class="text-sm font-bold text-rose-500 flex items-center gap-2">
                    <i class="fas fa-exclamation-triangle"></i> មូលហេតុនៃការលុបចោល
                </h3>
                <button @click="showCancelModal = false" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <p class="text-xs text-gray-600 dark:text-gray-300 bg-rose-50 dark:bg-rose-950/30 p-3 rounded-2xl border border-rose-100 dark:border-rose-900/30" x-text="cancelReason"></p>
        </div>
    </div>
</div>
...
<script>
    ...
    document.addEventListener('click', function(e) {
        ...
        if (link && link.href && (link.href.includes('page=') || link.href.includes('reportrooms'))) {
            e.preventDefault();
            const pageContainer = document.getElementById('reportrooms-page') || document.querySelector('[x-data*="fetchReports"]');

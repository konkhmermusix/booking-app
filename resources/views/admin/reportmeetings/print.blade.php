...
<body class="p-8 max-w-4xl mx-auto">
    ...
    <div class="no-print mb-6 flex justify-between items-center bg-gray-100 p-4 rounded-2xl border border-gray-200">
        <div>
            <h1 class="font-bold text-gray-800 text-sm">ផ្ទាំងព្រីនរបាយការណ៍សាលប្រជុំ (Print & Export Preview)</h1>
            <p class="text-xs text-gray-500">ចុចប៊ូតុងខាងស្តាំដើម្បីព្រីន ឬ រក្សាទុកជាឯកសារ PDF ទម្រង់ A4</p>
        </div>
        <div class="flex items-center gap-2">
            <button onclick="window.print()" class="px-5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold rounded-xl flex items-center gap-2 shadow-sm">
    ...
    <div class="flex justify-between items-center border-b-2 border-gray-800 pb-4 mb-6">
        ...
        <div class="text-right">
            <div class="inline-block p-3 bg-purple-50 border border-purple-200 rounded-xl mb-1">
                <h3 class="text-base font-black text-purple-900">របាយការណ៍កក់សាលប្រជុំ</h3>
            </div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
        ...
        <div class="border border-gray-300 p-3 rounded-xl text-center bg-gray-50">
            ...
            <p class="text-xl font-black text-purple-700 mt-0.5">${{ number_format($totalRevenue, 2) }}</p>
        </div>
    </div>
    ...
    <table class="w-full text-left border-collapse border border-gray-300 text-xs mb-8">
        <thead>
            <tr class="bg-gray-200 text-gray-800 font-bold border-b border-gray-300">
                ...
                <th class="p-2.5 border-r border-gray-300">ឈ្មោះអតិថិជន/អង្គភាព</th>
                <th class="p-2.5 border-r border-gray-300">សាលប្រជុំ / ប្រភេទ</th>
                <th class="p-2.5 border-r border-gray-300 text-center">ថ្ងៃប្រជុំ - ម៉ោង</th>
                <th class="p-2.5 border-r border-gray-300 text-center">អ្នកចូលរួម</th>
        ...
        <tbody class="divide-y divide-gray-300">
            @forelse($bookings as $index => $b)
                ...
                <tr>
                    ...
                    <td class="p-2.5 border-r border-gray-300 font-bold text-purple-900">#{{ $b->booking_code }}</td>
                    <td class="p-2.5 border-r border-gray-300 font-semibold">{{ $b->customer_name ?: ($b->user->name ?? 'ភ្ញៀវ Walk-in') }}</td>
                    <td class="p-2.5 border-r border-gray-300">សាល {{ $b->room->room_number ?? 'N/A' }}</td>
                    <td class="p-2.5 border-r border-gray-300 text-center font-mono">
                        {{ $b->start_date ? $b->start_date->format('d/m/Y') : '' }}
                    </td>
                    <td class="p-2.5 border-r border-gray-300 text-center font-bold">{{ $b->attendees_count ?: 0 }} នាក់</td>
                    <td class="p-2.5 border-r border-gray-300 text-center font-bold">
                        {{ $statusKhmer }}
                    </td>
                    <td class="p-2.5 text-right font-bold text-purple-900">${{ number_format($b->total_price, 2) }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="8" class="p-4 text-center text-gray-400 italic">មិនទាន់មានទិន្នន័យឡើយ</td>
                </tr>
            @endforelse
        </tbody>
        <tfoot>
            <tr class="bg-gray-100 font-bold text-gray-800 border-t border-gray-400">
                <td colspan="7" class="p-3 text-right uppercase">សរុបទឹកប្រាក់សរុប (Total Revenue):</td>
                <td class="p-3 text-right text-purple-950 font-black text-sm">${{ number_format($totalRevenue, 2) }}</td>
            </tr>
        </tfoot>
    </table>

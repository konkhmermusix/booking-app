    ...
    <aside
        ...
        <nav class="flex-1 py-6 px-3 space-y-1 overflow-y-auto no-scrollbar">
            ...
            <div class="relative"
                ...
                @mouseleave="if(!sidebarOpen) { hover = false; }"
                x-data="{ 
                    open: {{ request()->routeIs('users.*', 'contact.*', 'reviews.*', 'profile.*', 'notifications.*') ? 'true' : 'false' }},
                    hover: false,
                    topPos: 0
                }">
                <button @click="if(sidebarOpen) open = !open"
                    class="w-full flex items-center gap-4 p-3 rounded-xl hover:bg-white/10 text-gray-400 hover:text-white transition-all {{ request()->routeIs('users.*', 'contact.*', 'reviews.*','profile.*', 'notifications.*') ? 'bg-white/10 text-white' : '' }}"
                    :class="sidebarOpen ? 'justify-start' : 'justify-center'">
            ...
            <div class="flex items-center gap-3 md:gap-4">
                ...
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div x-show="open" x-cloak
                        ...
                        <a href="{{ route('profile.index') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-gray-700 transition">
                            <i class="fas fa-user-gear text-blue-500"></i> ការកំណត់គណនី
                        </a>

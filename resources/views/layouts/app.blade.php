        ...
        <nav class="bg-white/80 dark:bg-gray-950/80 backdrop-blur-md sticky top-0 z-100 border-b border-gray-100 dark:border-gray-800">
            <div class="container mx-auto px-4 md:px-6 py-3 flex justify-between items-center">
                ...
                <div class="flex items-center space-x-3 md:space-x-4">
                    ...
                    <a href="/booking"
                        class="hidden sm:flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-full text-xs font-bold hover:bg-blue-700 transition shadow-lg relative">

                        <i class="fas fa-calendar-check animate-bounce"></i>
                        {{ __('auth.nav-book') }}

                        <!-- បង្ហាញចំនួនលេខក្នុងកន្ត្រក នៅលើ Navbar តែម្ដង -->
                        <template x-if="count > 0">
                            <span x-text="count"
                                class="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full border-2 border-white shadow-md">
                            </span>
                        </template>
                    </a>

                    <!-- Floating Cart (សម្រាប់ Mobile ឬទុកមើលពេល Scroll ចុះក្រោម) -->
                    <div class="fixed bottom-10 right-10 z-50 sm:hidden" x-show="count > 0" x-transition>
                        <a href="/booking" class="bg-blue-600 text-white p-4 rounded-full shadow-2xl relative inline-block">
                            <i class="fas fa-shopping-cart text-xl"></i>
                            <span x-text="count" class="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] w-6 h-6 flex items-center justify-center rounded-full border-2 border-white"></span>
                        </a>
                    </div>

                    <div class="relative" x-data="{ open: false }">
                        <!-- ប៊ូតុងកន្ត្រក -->
                        <button @click="open = !open" class="relative p-2 text-gray-700 dark:text-white">
                            <i class="fas fa-shopping-cart text-xl"></i>
                            <span x-text="count" class="absolute top-0 right-0 bg-red-500 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full"></span>
                        </button>

                        <!-- ផ្ទាំងបង្ហាញបញ្ជី (Dropdown) -->
                        <div x-show="open" @click.away="open = false"
                            ...
                            <div class="p-4 bg-gray-50 dark:bg-gray-900/50">
                                <a href="/checkout" class="block text-center bg-blue-600 text-white py-2 rounded-xl text-sm font-bold">
                                    ទៅកាន់ទំព័រទូទាត់ប្រាក់
                                </a>
                            </div>
                        </div>
                    </div>
